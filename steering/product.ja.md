...
**Vision Statement**: 高品質かつ低コストな知識グラフベースRAGの実現

> Microsoft GraphRAGの品質を維持しながら、LazyGraphRAGのコスト効率（1/100以下）を実現する
> オープンソース実装を提供し、エンタープライズAIアプリケーションの民主化に貢献する

**Mission**: LazyGraphRAG固有のクエリアルゴリズムをオープンソースGraphRAGに完全実装

> 遅延LLM呼び出し、反復的深化検索、予算制御型関連性テストにより、
> インデックス作成とクエリ応答の両方でコスト効率を最大化する
...
Microsoft GraphRAGのオープンソース実装に、LazyGraphRAG固有のクエリアルゴリズムを追加するプロジェクトです。

> **GraphRAG**は、LLMを使用して非構造化テキストから知識グラフを構築し、
> RAG（Retrieval-Augmented Generation）操作を改善するMicrosoft Researchの技術です。
> 
